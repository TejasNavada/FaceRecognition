...
class TripletGenerator:
    ...
    def generate_all_people_dict(self):
        # create and empty dictinary that will be populated with
        # dictionary names as keys and miage names as values
        allPeople = dict()
        ...
        for personName in self.peopleNames:
            ...
            # build the image paths and populate the dictonary
            personPhotos = [
    ...
    def get_next_identity(self):
        while True:
            for person in self.peopleNames:
                identityPhoto = random.choice(self.allPeople[person])
                yield(identityPhoto, identityPhoto)
